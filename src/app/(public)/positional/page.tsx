...
export default async function PositionalPage({ searchParams }: Props) {
  ...
  function TradeCard({ group }: { group: (typeof filtered)[number] }) {
    return (
      <Link
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg font-semibold text-slate-900 truncate">{group.title}</h2>
            </div>
                ...
                return (
                  <div key={bundleId} className="space-y-3">
                    ...
                    <div className="space-y-3 pl-2">
                      {bundle.items.map((group) => (
                        <TradeCard key={group.id} group={group} />
                      ))}
                    </div>
                  </div>
                );
